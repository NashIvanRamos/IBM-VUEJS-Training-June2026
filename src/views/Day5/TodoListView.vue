...
const filteredTodos = computed(() => {
  ...
  if (filter.value === 'done') return todos.value.filter(todo => todo.completed).slice(0, 20)
  if (filter.value === 'pending') return todos.value.filter(todo => !todo.completed).slice(0, 20)
  return []
})
</script>
...
<template>
  <div class="todo-view">
    ...
    <div v-else>
      ...
      <div class="count">
        Showing {{ filteredTodos.length }} {{ filter === 'all' ? 'todos' : filter }} items
      </div>
    </div>
  </div>
</template>
...
.filters button { padding: 6px 16px; border: 1px solid #ddd; border-radius: 20px; background: white; cursor: pointer; font-size: 13px; color: #1B2A4A; font-weight: 500; }
